...
# Данные о самолете
mass = 45880        # Масса самолета, кг
thrust = 137200     # Общая тяга всех двигателей, Н
Cx = 0.0610         # Коэффициент продольного сопротивления (лобовое сопротивление)
Cy = 0.828          # Коэффициент поперечного сопротивления (подъемная сила)
area = 77           # Площадь крыла, м²
density_air = 1.225 # Плотность воздуха, кг/м³
takeoff_angle = 10  # Угол подъема в градусах
...
while current_speed < 56:  # Цикл продолжается пока скорость меньше 56 м/с
...
plt.title("График дистанции взлета SJ-100 (Профиль ЦАГИ-6-12%)")
plt.legend()
plt.grid(True)
plt.show()